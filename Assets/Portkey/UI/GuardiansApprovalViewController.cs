...
namespace Portkey.UI
{
    public class GuardiansApprovalViewController : MonoBehaviour
    {
        ...
        public void InitializeData(Action onCompleted, ErrorCallback errorCallback)
        {
            ...
            StaticCoroutine.StartCoroutine(did.GetVerifierServers(_guardianIdentifierInfo.chainId, StoreVerifierServers,
                _onErrorInitData));
        }
        ...
        private void GetGuardianStatusList(IReadOnlyDictionary<string, VerifierItem> verifierMap)
        {
            ...
            {
                var guardians = (result?.guardianList == null)
                    ? Array.Empty<Guardian>()
                    : result.guardianList.guardians;
                var guardianStatusMap = _guardianStatusList.ToDictionary(
                    status =>
                        $"{(status.guardianItem.identifier ?? status.guardianItem.guardian.identifierHash)}&{status.guardianItem.guardian.verifierId}",
                    status => status);
                var approvedMap =
                    _approvedGuardians.ToDictionary(approval => $"{approval.identifier}&{approval.verifierId}",
                        approval => approval);
                ...
                foreach (var guardian in guardians)
                {
                    ...
                    newGuardianStatus =
                        UpdateStatusIfApproved(GetGuardianKey(guardian), newGuardianStatus, approvedMap);
        ...
        private void UpdateGuardianInfoUI()
        {
            ...
            var isCompletedVerification =
                VerifiedCount(_guardianStatusList) >= did.GetApprovalCount(_guardianStatusList.Count);
        ...
        {
            _guardianItemComponents.Where(item => item.VerifierStatus != VerifierStatus.Verified).ToList()
                .ForEach(item => item.SetExpired(true));
        }

        private void CompleteVerificationForGuardianItems()
        {
            _guardianItemComponents.Where(item => item.VerifierStatus != VerifierStatus.Verified).ToList()
                .ForEach(item => item.SetEndOperation());
        }
        ...
        private UserGuardianStatus CreateUserGuardianStatus(Guardian guardian,
            IReadOnlyDictionary<string, UserGuardianStatus> guardianStatusMap)
        {
        ...
        private static bool IsMatchingGuardianIdentifier(GuardianIdentifierInfo guardianIdentifierInfo,
            GuardianItem guardianItem)
        {
            return guardianIdentifierInfo.token != null &&
                   guardianIdentifierInfo.identifier == guardianItem.identifier &&
                   guardianIdentifierInfo.accountType == guardianItem.guardian.type;
        }
        ...
        {
            foreach (var status in _guardianStatusList.Where(status =>
                         IsMatchingGuardianIdentifier(_guardianIdentifierInfo, status.guardianItem)))
            {
